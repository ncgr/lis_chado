...
If using GMOD database profile:
	 ...
	 perl gmod_bulk_load_bp_v2_pass2.pl -H [dbhost] -D [dbname]  [-vt] -i file

If not using GMOD database profile: 

	perl gmod_bulk_load_bp_v2_pass2.pl -H [dbhost] -D [dbname] -d [database driver] -u [user name] -p [password] -i [input_file] 
...
#use lib '/home/peu/local_Bio/lib/';
use Getopt::Std;
...
#****testing samples for project PRJNA197379 example****#
...
eval {  
		...
		'Description/Organism'    => \&organism_s, #3
                ...
                'Attributes/Attribute'   => \&attribute,#5 #populates biomaterialprop table
		'Ids/Id' => \&other_dbs,  #1
		},
...
$biomaterial_s->dbxref_id($a);
$biomaterial_s->stock_id($stock_id);
...
elsif ($attr_simple eq 'strain' || $attr_simple eq 'cultivar')
{
...
$stock = $schema->resultset('Stock::Stock')->find_or_create({name=>$ele_c, uniquename=>$ele_c, type_id=>$type_c, organism_id=>$b, description=>$attr_simple});
$stock_id = $stock->get_column('stock_id');
...
print "Material organism is: $b \n\n";  
          ...
          $biomaterial = $schema->resultset('Mage::Biomaterial')->find_or_new({ name => "$acce-$name"}); #biomaterial name is unique
           unless ($biomaterial->in_storage) {
					...
					#	$biomaterial->biosourceprovider_id($a);
						$biomaterial->taxon_id($b);
...
#*****newly added sub for project type (type_id)
