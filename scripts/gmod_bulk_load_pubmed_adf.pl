...
eval {
	...
	# new Bio::Chado::Schema::Pub::Pub object
        #adf: ? technically, couldn't there be multiple?
	my $pub_dbxref = $dbxref->find_related ('pub_dbxrefs', {dbxref_id => $dbxref->id}, );
        if ($pub_dbxref) {
            $publication= $pub_dbxref->find_related('pub', {id => $pub_dbxref->pub_id}, ) if $pub_dbxref; #add in my script
        }
